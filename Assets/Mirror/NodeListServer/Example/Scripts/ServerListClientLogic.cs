...
namespace NodeListServer
{
    public class ServerListClientLogic : MonoBehaviour
    {
        ...
        [Tooltip("The URL to connect to the NodeListServer. For example, http://127.0.0.1:4296/list.")]
        [SerializeField] private string masterServerUrl = "http://172.20.10.5:4296/list";
        [Tooltip("The key required to talk to the server. It must match. Default is NodeListServerDefaultKey.")]
        [SerializeField] private string communicationKey = "mirror4296";
        ...
        private NodeListServerNetManager nodeServer;
        ...
        private void Start()
        {
            nodeServer = FindObjectOfType<NodeListServerNetManager>();
            if (refreshOnStart)
            {
                RefreshList();
            }
